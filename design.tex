...
mitigates the impact of bugs in experimenter code.}. 
The sandbox is comprised of a small, self-contained  
kernel as its trusted computing base (TCB), and places sensitive
portions of libraries (such as networking and filesystem 
functions, locking and threading functions, etc.) in 
unprivileged, isolated components. Experiment code has a common 
language interface~\cite{repyv2} to access resources on a device, 
with full transparency. Vulnerabilities in a library can 
only cause compromise in the library, and the experiment code 
can at most gain capabilities granted to the library. Over the past 6 
years, we have used the described sandbox as part of a 
network research testbed~\cite{seattle}. Our experience has 
shown that the risk of it being faulty is very low.

\yanyan{need a transition from small TCB to interposing system calls.}
