...
This section addresses how these parties interact to
...
type (3G, 4G, LTE, etc.), and signal strength. A usage scenario, illustrating how the components operate follows each description.
...
Alice downloads 
our Sensibility Testbed app from the Google Play Store~\cite{sensibility-app}, which installs Repy and other software on her phone.
...
volunteer \textit{once}, at the time of app installation. As a result, an 
...
experiment. \lois{is the previous sentence needed? I don't think so} The testbed thus greatly simplifies the process for both the 
...
keys with values. \lois{previous sentence is unclear} Through the app interface, Alice may uninstall, or 
choose to opt out of individual experiments. \lois{how? I have not seen this explained yet and I think its critical to discuss this}
...
the set of devices that he has access to.\lois(have you introduced the idea of keys yet? If not, I think this needs to be explained.)
...
and access frequency limits. \lois{I think I already asked this, but is it literally just the sensors that the page shows, and not the devices containing the sensors that are shown? It's a small point, but I think it has to be clarified} In this case, Bob specifies that 
...
moderate risks. (\tickmark). Sensors that are highly risky are 
