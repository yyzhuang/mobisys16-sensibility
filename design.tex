...
mitigates the impact of bugs in experimenter code.}. The sandbox 
is comprised of a small, self-contained kernel as its trusted 
computing base (TCB), unprivileged libraries and experiment code. 
Each system call in the TCB is strictly 
sanitized to preserve consistent behavior across difference OSes, 
and avoid uncommon corner cases that can be exploited. Since 
the TCB is small (about 8,000 lines of code), it is less likely to have
a kernel exploit than other more complex kernels.
Experiment code uses the programming language 
interface~\cite{repyv2} provided by the sandbox to access 
resources on a device. Any vulnerability in an experiment can 
at most cause compromise in the unprivileged portion of the sandbox, 
and cannot escape the sandbox and perform malicious actions to 
the device system. Over the past 6 years, we have used the 
described sandbox as part of a network research 
testbed~\cite{seattle}. There has been no report of security 
breaches or the sandbox being faulty.
...
interposing on system calls that use resources, such as network and disk I/O, and 
