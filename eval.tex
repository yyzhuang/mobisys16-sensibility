...
\hline
{\bf Project} & {\bf Sensor} & \multicolumn{3}{c|}{\bf Sensibility Testbed policy}  \\\hline

EnCore~\cite{aditya2014encore}  & Bluetooth & Randomize Bluetooth MAC address. &   &   \\\hline
...
\multirow{2}{.8cm}{FindingMiMo \cite{shin2011findingmimo}} 
& WiFi & \multirow{2}{*}{Access frequency $\leq$ once per 2 minutes.} &   &  \\ \cline{2-2}
& Location & &  & \\\cline{2-5}
& Accelerometer & \multirow{2}{*}{Not specified, only for move detection.} &   &  \\ \cline{2-2}
& Magnetometer &  &   &  \\ \hline
...
\multirow{2}{*}{TapLogger~\cite{xu2012taplogger}} & Gyroscope & \multirow{2}{*}{Access frequency $\leq$ 25 Hz.} &   &  \\ \cline{2-2}
& Accelerometer &  &   &  \\ \hline
...
Where's Wally \cite{polakis2015s}%\textsuperscript{*}
& Location & \tickmark &   &  \\ \hline
% attack: tracking and localization attacks
...
\caption{\small Sensibility Testbed's support against known attacks. 
...
\textbf{Protection Against Known Attacks.}
...
devices, we surveyed 31 projects in the past years by reading their corresponding 
publications. Some of these projects
propose privacy protection mechanisms against a certain attack (e.g., 
EnCore~\cite{aditya2014encore} uses randomized Bluetooth MAC address to 
prevent the tracking of a device), others show the posibility of a certain attack 
using sensors (e.g., TapPrints~\cite{miluzzo2012tapprints} shows that it is 
possible to infer key strokes using motion sensors). We analyzed both kinds of
projects, and identified the privacy risks in these projects.
\todo{Q: what privacy concerns do people have? }

These projects are listed in Table~\ref{tab:policy}. As shown in the table, the 
most common risks are: identifying a device or its owner, locating a device, and 
inferring keys strokes typed by a device owner. In this table, we also show the
policies in \sysname that can be applied to protect the device owner from the
attacks stated in the project publication. For example, 
FindingMiMo~\cite{shin2011findingmimo} locates a device by monitoring 
the device's location and ambient radio signal (particularly WiFi). It generates 
an ambient environment fingerprint that a device encouter during a day so 
that the device can be located. As shown in Section 5.1 
of~\cite{shin2011findingmimo}, the GPS location and WiFi scan's sampling 
interval is 2 minutes. Therefore, restricting the access frequency to these 
sensors to below once per 2 minutes can effectively prevent the environment 
fingerprinting, even if the accuracy of GPS and WiFi is not reduced.
...
\caption{\small \sysname's support for privacy protection compared to other
privacy frameworks.}
...
\textbf{Are Sensibility Testbed's Policies Sufficient?}
We compare \sysname's privacy protection with other privacy frameworks. 
