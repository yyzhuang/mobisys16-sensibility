...
the device owner as a ``web app''.\yanyan{what does it mean to make
the first move?}}. 
...
of this type, or handle phone calls, etc.\yanyan{Why would a device 
owner want to open a file using testbed code? Can't they use the Android
built-in player? Do you mean a researcher
wants to open an MP3 file on device owner's device?}
...
\cite{it} \yanyan{what is this paper?} claims \textit{tons of} installs from the Android app 
...
improve the coverage provided by the blurring layers.\yanyan{cite software updater url.}
