...
program that is configured when the \sysname app 
is installed. \yanyan{what does the configuration do?} As with the sandbox, the device manager is based on our 
...
might be unmounted and ejected from the device at any time. \yanyan{how is this handled??}
...
To enable local configuration of policies set by a device owner, 
the device manager parses the input from the user interface
\yanyan{add how device manager policies --> sandbox policies.}

\textbf{Clearinghouse.}
\sysname's clearinghouse is hosted on a web server at NYU. 
\yanyan{cite, but sensibilityclearinghouse.poly.edu/ does not have 
registration form. Kaushik's website is ugly..}
%\albert{...trying to indicate that ``you, dear reader'' needn't worry about that aspect...}
...
comes out of blue. or ref to TR \cite{zhuangTR15}?} and 
sandbox control as the other parts of the system.

The clearinghouse uses databases to store the information about
available devices, and registered researcher's experiments. \yanyan{briefly 
describe the DBs, how the devices get timed out when inactive, how 
to pass the parsed IRB policies onto device sandboxes, etc.}
...
the development of experiments (\yanyan{such as libraries to...}), is available 
